...
| **OpenTofu** | `OpenTofu controller guest firewall` | **Secure Note**, concealed **`state_json`**: OpenTofu state for **`iac-controller-guest-firewall.tfstate`** (`proxmox_virtual_environment_firewall_rules`). **Create this item** in the OpenTofu vault before bootstrap (same field name as the main state item). Bootstrap **imports** when local state or the resource is missing, **plans**, **applies** if the plan has changes, then **pushes** updated state via Connect. |
