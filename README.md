...
| **IaC Controller** | `Cursor SSH Public Key` | **Secure Note**, string field **`public_key`**: one line **SSH public key** (same as your `.pub` file for Cursor / Remote SSH). The playbook **reads** it into **`cursor`**’s **`authorized_keys`**; it never generates Cursor keys. Required before bootstrap can enable SSH. |
