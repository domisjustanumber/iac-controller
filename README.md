...
| `ansible/ansible.cfg` | `allow_world_readable_tmpfiles` mitigates Ansible 2.20 ACL-style temp modes with **`become_user`** on **localhost**. Ephemeral OpenTofu dirs use **root `tempfile` + `chown tofu`** to avoid that path. The bootstrap script sets **`ANSIBLE_CONFIG`** to this file. |
...
|-------|------------|---------|
| **IaC Controller** | `GitHub App` | Concealed **`Private_Key`**: GitHub App **private key** from Developer Settings → Private keys — **PEM** (`BEGIN PRIVATE KEY` / `BEGIN RSA…`) or **OpenSSH** (`BEGIN OPENSSH PRIVATE KEY`); must be **unencrypted**. Paste preserves real newlines (not the two-char sequence `\n`). **`Client_ID`**: **Client ID** string (JWT `iss`), not the numeric App ID. Optional **`Installation_ID`**. |
