...
Secrets are split across **three** vaults (exact names in `ansible/inventory/group_vars/all.yml`: `iac_iac_controller_vault`, `iac_ansible_vault`, `iac_opentofu_vault`). The **bootstrap** Connect token must be able to read (and create where needed) items in **all three** during `iac_controller.yml`. After success, **`opentofu`** uses a token whose items live in **`OpenTofu`**; future **`ansible-playbook`** runs use a token scoped to **`Ansible`**. Titles are configurable in `group_vars`; defaults:
...
| **IaC Controller** | `1Password Connect Access Token: Bootstrap` | *(Reference; value is pasted on the PVE host.)* Concealed **`credential`** item can live here for documentation alongside other bootstrap secrets. |
| **Ansible** | `1Password Connect Access Token: Ansible` | Concealed **`credential`**: Connect token for ongoing Ansible runs. Bootstrap reads it **from the Ansible vault** and writes `/home/ansible/.config/op/connect_token` after a **successful** run. |
| **Ansible** | `Ansible/SSH_Key` | **Password** item: concealed **`password`** (PEM) and string **`public key`**. Bootstrap creates or updates this in the **Ansible** vault and installs **`/home/ansible/.ssh/id_ed25519`** (+ `.pub`). |
| **OpenTofu** | `1Password Connect Access Token: OpenTofu` | Concealed **`credential`**: Connect token for **`opentofu`**. Bootstrap reads it from the **OpenTofu** vault and writes `/home/opentofu/.config/op/connect_token`. |
| **OpenTofu** | `Ansible SSH Key` | **Secure Note**, string **`public_key`**: controller Ansible **public** line for cloud-init / `authorized_keys`. Synced from the key in the Ansible vault when missing or mismatched. |
...
| **OpenTofu** | `OpenTofu/state_file` | Concealed **`state_json`**: optional state blob when no local `terraform.tfstate` yet (subject to 1Password size limits). |
