...
| `Ansible/SSH_Key` | **Password** item with concealed **`private key`** and string **`public key`** fields. If the item or **`private key`** field is missing, bootstrap generates an ed25519 keypair, stores both halves via `generic_item`, and installs **`/home/ansible/.ssh/id_ed25519`** (+ `.pub`). Connect does not support creating `SSH_KEY`-category items, so a Password item is used with the same field labels for `field_info` reads. |
| `OpenTofu/Ansible SSH Key` | **Secure Note** with string field **`public_key`**: same OpenSSH **public** line as the controller’s Ansible key. The playbook **creates or updates** this item whenever the value is missing or differs, so **OpenTofu** (or CI) can inject it into new VMs/LXCs (e.g. cloud-init / `authorized_keys`) while Ansible keeps using the **private** key on the controller. See `examples/deployment/ansible_ssh_on_proxmox_guests.tf.example`. |
| `OpenTofu/PVE API Key` | Concealed field **`credential`**: full Proxmox API token value (`user@realm!tokenid=secret`). The playbook pushes it from **`/home/opentofu/.config/iac-controller/pve_api_token`** (bootstrap-only staging) with the **bootstrap** Connect token, then **removes that file only after a successful 1Password sync** so the secret is not left on disk. |
