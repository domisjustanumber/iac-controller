...
| `Ansible/SSH_Key` | **[SSH Key](https://developer.1password.com/docs/cli/ssh-keys/)** item (`SSH_KEY` in the [Connect API](https://developer.1password.com/docs/connect/api-reference/)): bootstrap sends only the **`private key`** field (`SSHKEY`); **1Password derives** the public key, fingerprint, and key type. The playbook then uses the returned item (or a follow-up read) for **`/home/ansible/.ssh/id_ed25519.pub`**. |
