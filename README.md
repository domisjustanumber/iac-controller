...
| **Ansible** | `Ansible SSH Key` | **SSH Key** item by default: **`POST`** to Connect **`/v1/vaults/{uuid}/items`** with category **`SSH_KEY`**, private PEM in **`private key`** only; 1Password fills **`public key`**, which bootstrap reads back (optional **`GET`** refresh, then local **`.pub`** if still empty). Uses **`ansible.builtin.uri`** because **`generic_item`** does not support **`ssh_key`**. **Rescue**: legacy **Password** item (**`password`** + **`public key`**). **Reads** either private field for existing items. Installs **`/home/ansible/.ssh/id_ed25519`** (+ `.pub`). |
| **OpenTofu** | `Ansible SSH Public Key` | **SSH Key** item via Connect (**`SSH_KEY`**): concealed **`private key`** + concealed **`public key`** (same labels as the Ansible vault key). Replaces legacy **Secure Note** `string` **`public_key`** when missing, mismatched, or item category is not **SSH Key**. For OpenTofu to inject into new VMs (cloud-init / `authorized_keys`). |
