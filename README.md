...
4. **SSH**: The PVE script **stops and masks** `ssh` until **`iac_controller.yml`** finishes. The playbook then enables **`openssh-server`** with **`/etc/ssh/sshd_config.d/99-iac-controller.conf`**: **`AllowUsers cursor`**, **public-key (or SSH certificate) only** (`AuthenticationMethods publickey`, no password auth), **`PermitRootLogin no`**. The **`cursor`** user’s **`authorized_keys`** line comes from 1Password (**IaC Controller** vault, Secure Note **`Cursor SSH Key`**, field **`public_key`**); **`cursor`** has passwordless **`sudo`**. Use **`pct exec`**, the Proxmox console, or **`ssh cursor@…`** (controller hostname or IP) after bootstrap.
...
| **IaC Controller** | `Cursor SSH Key` | **Secure Note**, string field **`public_key`**: one line SSH **public key** or **OpenSSH user certificate** for the **`cursor`** user (`authorized_keys`). Required before bootstrap can enable SSH. |
...
# or: ssh -i ~/.ssh/your_cursor_key cursor@<controller-hostname-or-ip>
```
